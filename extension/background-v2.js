import { analyze } from './engine-v2.js';
...
const normalizeQuery = (value) => String(value || '').replace(/^\$/, '').trim().toLowerCase();
...
async function resolveVisibleItem(item) {
  ...
  const candidates = pairs.filter((pair) => pair?.chainId === 'solana' && pair?.baseToken?.address).map((pair) => {
    ...
    return { pair, score, marketCap, price, liquidity };
  }).filter(Boolean).sort((a,b) => b.score - a.score);

  if (!candidates.length) { resolveCache.set(cacheKey,{at:Date.now(),result:null}); return null; }
  ...
  if (best.score < 56 || (!anchored && margin < 12) || (anchored && margin < 5)) {
    resolveCache.set(cacheKey,{at:Date.now(),result:null}); return null;
  }

  const result = {
    token: String(best.pair.baseToken.address), query,
    ...
    marketCapUsd: best.marketCap, priceUsd: best.price, liquidityUsd: best.liquidity,
    resolutionScore: Math.round(best.score), resolutionMargin: Math.round(margin),
...
async function resolveVisibleMany(items) {
  ...
  async function worker() {
    while (cursor < queue.length) {
      const index = cursor++;
      try { const result = await resolveVisibleItem(queue[index]); if (result) resolved.push(result); } catch { /* retry later */ }
    }
  }
  await Promise.all(Array.from({length:Math.min(4,queue.length||1)},worker));
  const byQuery = new Map();
  for (const item of resolved) {
    const key = normalizeQuery(item.query || item.symbol), current = byQuery.get(key);
    if (!current || item.resolutionConfidence > current.resolutionConfidence || (item.resolutionConfidence === current.resolutionConfidence && item.resolutionScore > current.resolutionScore)) byQuery.set(key,item);
  }
...
async function radarMany(items) {
  ...
  async function worker() {
    while (cursor < queue.length) {
      const index = cursor++;
      try { results[index] = await radarOne(queue[index]); }
      catch (error) { results[index] = {tokenAddress:queue[index]?.token||'',error:error instanceof Error?error.message:String(error),priority:0,risk:100,status:'UNAVAILABLE',dataCoverage:0}; }
    }
  }
  await Promise.all(Array.from({length:Math.min(4,queue.length||1)},worker));
...
chrome.runtime.onMessage.addListener((message,_sender,sendResponse)=>{
  if (message?.type === 'NEO_ANALYZE') {
    analyze(message.input,message.pageContext||{}).then((result)=>sendResponse({ok:true,result})).catch((error)=>sendResponse({ok:false,error:error instanceof Error?error.message:String(error)}));
    return true;
  }
  if (message?.type === 'NEO_RADAR_SCAN') {
    radarMany(message.items).then((results)=>sendResponse({ok:true,results})).catch((error)=>sendResponse({ok:false,error:error instanceof Error?error.message:String(error)}));
    return true;
  }
  if (message?.type === 'NEO_RESOLVE_VISIBLE_TOKENS') {
    resolveVisibleMany(message.items).then((results)=>sendResponse({ok:true,results})).catch((error)=>sendResponse({ok:false,error:error instanceof Error?error.message:String(error)}));
    return true;
  }
  return false;
});
