...
async function send(message,repair=true){
  ...
  try{return await chrome.tabs.sendMessage(activeTabId,message);}catch(error){
    if(!repair||repairingConnection)throw error; repairingConnection=true; statusText.textContent='Активирам Sentinel v0.7…';
    try{await chrome.scripting.executeScript({target:{tabId:activeTabId},files:['sentinel-v3.js']});await sleep(700);return await chrome.tabs.sendMessage(activeTabId,message);}finally{repairingConnection=false;}
  }
}
function render(r){
  ...
  if(r.deepScanning)statusText.textContent='Deep анализът върви; realtime market scan продължава паралелно.';
  else if(r.radarScanning)statusText.textContent='NEO обновява market flow и liquidity.';
  else if((r.observedCount||0)>0)statusText.textContent='Multichain Sentinel е активен и наблюдава candidates автоматично.';
  else statusText.textContent='Sentinel е активен и търси token-и по адрес, име, symbol и market context.';
...
rescanBtn.addEventListener('click',async()=>{clearError();rescanBtn.disabled=true;rescanBtn.textContent='ОБНОВЯВАМ…';try{if(!activeTabId)await readStatus();if(activeTabId){await send({type:'NEO_RESCAN'},true);setTimeout(readStatus,1000);setTimeout(readStatus,3000);}}catch(e){showError(e instanceof Error?e.message:String(e));}finally{setTimeout(()=>{rescanBtn.disabled=false;rescanBtn.textContent='ОБНОВИ SENTINEL';},1000);}});
